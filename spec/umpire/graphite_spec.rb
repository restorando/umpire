...
describe Umpire::Graphite do
  ...
  describe "get_values_for_range" do
    ...
    it "should return an Array" do
      ...
      Umpire::Graphite.get_values_for_range(metric, range).should be_kind_of(Array)
    end
    ...
    it "should return the expected values" do
      stub_request_with_values
      Umpire::Graphite.get_values_for_range(metric, range).should eq([4.47])
    end

    it "should raise an exception if graphite returns empty data" do
      stub_request_without_values
      lambda { Umpire::Graphite.get_values_for_range(metric, range) }.should raise_error(MetricNotFound)
    end
    ...
    it "should raise an exception if the graphite HTTP request fails for any reason" do
      ...
      lambda { Umpire::Graphite.get_values_for_range(metric, range) }.should raise_error(MetricServiceRequestFailed, /timeout/i)
    end
  end
